...
export function getConversionOptionsFromColor(color) {
  let result;
...
export default async function convertColor(initialColor) {
  ...
  if (initialStage === 1) {
    if (!converter.isValidColor(initialColor)) {
      console.log(
        c.red("Invalid color. Run `convert-color --help` for usage.")
      );
      return;
    }
  }
  ...
  if (initialStage === 0) {
    ...
    let { color } = await enquirerObj.prompt({
      type: "input",
      name: "color",
  ...
  if (!converter.isValidColor(initialColor)) {
    console.log(
      c.red(
        "Invalid color. Please enter a valid hex, rgb, or hsl string. Run `convert-color --help` for usage."
      )
    );
    return;
  }
  ...
  let { outputType } = await enquirerObj.prompt({
    type: "select",
    name: "outputType",
  ...
  if (output === undefined) {
    ...
    console.log(
      c.green(
        `Converted value is ${output}. It has been copied to your clipboard.`
      )
    );
  }
}
